...
`.` 永遠指 chat 區域變數，`$` 永遠指全域變數。兩個 scope 不共用命名。
...
| <code v-pre>{{random::a::b::c}}</code> | 隨機一項。每次繪製都重擲。 |
| <code v-pre>{{pick::a::b::c}}</code> | 隨機一項，但**對同一 chat 同一位置穩定**。Seed = chat hash + content hash + 位置 + reroll seed。用 `/reroll-pick` 重置。 |
