...
1. 生成前呼叫 `startInspection` 記錄請求中繼資料
2. 生成完成後呼叫 `completeInspection` 記錄 Token 用量
3. 生成失敗時呼叫 `failInspection` 記錄錯誤資訊
...
- 從串流事件中提取 Token 用量資訊
- 統一的串流中斷和恢復處理
- 與 [WebSocket 代理](/zh-TW/improvements/other#websocket-代理)配合支援串流偏移恢復
