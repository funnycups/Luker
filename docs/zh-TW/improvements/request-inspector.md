...
請求記錄保存在記憶體中，每個使用者最多保留 200 筆。每個使用者只保留最近的記錄；達到上限後，最早的記錄會被移除騰出空間。

已完成的記錄保留兩小時，之後移除。仍在生成中的記錄不受此時限約束：進行中的請求最長保留六小時，長時間生成或停滯的串流不會在完成回呼執行前被清理。過期記錄每五分鐘清掃一次，因此一筆記錄最多在發起後保留兩小時零五分鐘。

兩個數值都可以在 `config.yaml` 中調整：
...
- `ttlMs`：已完成記錄的保留時長（毫秒）
- `cleanupIntervalMs`：過期清掃的執行間隔（毫秒）
...
> [!TIP]
> 請求檢查器會隨伺服器自動啟用，預設值開箱即用；需要調整時再改上面的保留設定即可。
