...
2. **API 預設** + **提示詞預設** 選 Step 2 派生的那份 `-orchestrator` 預設
3. 找到 **AI 迭代工作台** 區域，把它的 **API 預設** + **提示詞預設** 也填一份——但這裡**不要**用 `-orchestrator`，用一份**精簡到只剩越獄**的預設（本文件附帶的 [`plugin-only.json`](/presets/plugin-only.json) 就是為這種場景準備的）

::: tip 兩路預設分工
| 路徑 | 給誰用 | 選什麼 |
|---|---|---|
| **Agent API + 提示詞預設** | 編排器裡實際跑的主 Agent + 子 Agent，直接產出正文 | Step 2 派生的 `-orchestrator` 預設（自帶越獄、NSFW、文風指導） |
| **迭代工作台 API + 提示詞預設** | 你在工作台裡跟 AI 對話調編排時，工作台 AI 自己用 | `plugin-only.json` 或類似的純淨預設——它的工作是修改你的編排配置，不需要文風、越獄 |

Luker 內建的其他外掛（記憶圖 Schema 工作台、CardApp Studio、預設助手自身……）的「迭代 AI」也都建議掛這份精簡預設。
:::
...
記憶圖面板裡的 **Schema 迭代提示詞（schema-editor AI）** 那一欄走的就是 Step 3 提到的「迭代 AI 路徑」——這裡也建議掛 `plugin-only.json` 這樣的精簡預設。
:::
...
想看更細——每個 Agent 的具體模型思考、每次工具呼叫的請求和回應？編排器面板裡點 **查看執行態軌跡**。
