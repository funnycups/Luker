...
1. **查清執行時 ctx 表面。** 工作台先用 `luker_ctx_describe` 鑽入執行時 ctx（必要時 `luker_docs_read` 翻閱相關文件），把工具要碰到的資料形狀摸清楚——不靠猜。
2. **起草函式本體並 dry-run。** 工作台用 `luker_orch_dry_run_custom_tool` 給函式本體餵樣本入參，在沙箱裡跑一遍，讓任何執行時例外在你被請求批准之前先暴露出來。
3. **落地校驗通過的工具。** 工作台透過 `luker_orch_set_custom_tool` 掛卡；工具採用 read 模式（唯讀、無副作用），內部一段 JavaScript 讀取本輪的草稿快照、掃描尾部標籤。模式對應的啟用開關會被自動切到 true，下一輪主代理就能看見這個工具。
4. **改主代理的系統提示詞。** 工作台再呼叫 `luker_orch_patch_director_main_agent_system_prompt`（或 loop / agenda 的對應工具）追加一條規則：生成的最後一步必須調用該工具，若結果顯示缺標籤則就地補全後再產出。
