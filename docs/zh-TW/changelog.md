...
- **edits-lib 用法現在有兩條文件化的路徑**：Path 1（迭代工作台外殼適配器）適合彈窗形式的介面；Path 2（library-only）適合全屏 / 自定義 UI。參見 `docs/zh-TW/development/extension-api/edits-lib.md` 的「Path 2: library-only」一節。CardApp Studio 是 Path 2 的倉庫內參考實作。
