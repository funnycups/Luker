...
- **狀態追蹤** — 透過聊天變數(`ctx.getVariable` / `ctx.setVariable`)持久化好感度、任務進度、物品清單等資料;只有當資料是 CardApp 獨佔、不適合放進單個變數的結構化命名空間時,才用聊天狀態 sidecar(`ctx.getChatState` / `ctx.updateChatState`)。
