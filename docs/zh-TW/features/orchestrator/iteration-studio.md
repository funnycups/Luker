...
在角色卡專屬編排會話裡打開工作台時，AI 還會順便對照這張卡綁定的世界書。如果某條條目硬性約束了輸出（強制固定格式、要求純文字、禁用 markdown、固定回合結構），且這種約束和你正在搭建的編排的輸出契約衝突，工作台會就地調整這條條目——通常是直接停用（保留正文，方便日後重新啟用），少數情況會改寫幾句正文以放寬約束。每次調整都會和編排提案一起出現在會話歷史裡。
