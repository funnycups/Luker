...
在角色卡專屬編排會話裡打開工作台時，AI 還會順便對照這張卡綁定的世界書。如果某條條目硬性約束了輸出（強制固定格式、要求純文字、禁用 markdown、固定回合結構），且這種約束和你正在搭建的編排的輸出契約衝突，工作台預設不會直接停用整條——條目裡的格式約束往往承載著作者的認知意圖（場景錨點、人設節奏、敘事韻律、世界觀事實），整條停用會把這部分意圖也一起抹掉。所以工作台首選只改寫衝突那一句、保留條目的其它資訊載荷；只有當整條幾乎就是純格式約束（除了一句「必須用 markdown」以外沒別的）時才直接停用整條。任何情況都不會刪除條目。每次調整都會和編排提案一起出現在會話歷史裡。
