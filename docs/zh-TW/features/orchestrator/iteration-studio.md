...
- **AI 驅動收尾（Loop）。** AI 自行決定何時停止迭代——`continueRequested` 旗標位由 AI 透過工具呼叫控制，**沒有手動 Auto-Continue 開關**。
...
工作台 AI 讀你當前的 profile，透過工具呼叫產出 patch。每次工具呼叫要麼是 `luker_orch_continue_iteration`（繼續追問 / 局部更新），要麼是 `luker_orch_finalize_iteration`（這次需求已滿足）——AI 自行決定何時收尾。
