...
- `voice_critic`:口吻 / 角色一致性 —— 挑出感覺「不像這個角色」的句子。
- `continuity_critic`:對照聊天 / 記憶 / 世界書的連續性 —— 指出與已確立設定衝突的地方。
...
預設主代理系統提示詞與預設的五個子代理 id **強耦合**——它已經按「先派偵察、起草、再派評審、迭代修訂」的紀律調好了。要改的話推薦用 [AI 迭代工作台](/zh-TW/features/orchestrator/iteration-studio)用自然語言描述需求,讓它透過工具呼叫 patch 你的 profile。
:::
...
4. **預設 profile 自帶 5 個為 RP 最佳化過的子代理**:
   - 前置偵察(可並行派,各掃一個源):`chat_scout`(掃聊天)、`memory_scout`(掃記憶圖)、`lorebook_scout`(掃世界書)。
   - 後置評審(草稿後可並行派):`voice_critic`(口吻 / 角色一致性)、`continuity_critic`(連續性)。
   - 預設主代理系統提示詞與這五個 id **強耦合**,按 id 指名排程,併為每個寫好了 task brief 的樣式。改子代理時,主代理提示詞也要同步改。
...
- **系統提示詞** —— 留空使用內建預設(草稿 → 評審 → 修訂 + 派遣啟發式)。除非有明確理由,預設值是按「強制 critique 紀律」調校過的,值得先用預設跑兩輪再決定要不要改。
...
| 提示詞項 | 為什麼關 |
|---|---|
| **角色卡設定**(character description / personality / scenario / first message / example messages) | 主代理已經從聊天曆史看到了這個角色,前置偵察子代理也會主動把相關條目摘出來——再疊一層角色卡佔位符等於讓主代理的 prompt 多讀一遍同樣的內容。 |
| **使用者人設**(persona) | 同上,使用者的人設資訊已經在聊天上下文裡;佔位符再注一份只會讓主代理在「這一回合用戶是誰」這件事上看到兩份可能彼此矛盾的描述。 |
| **示例對話**(example messages) | 示例對話本意是給主對話的主 LLM 看「角色的說話風格長什麼樣」。但 Director 的最終正文不是主 LLM 寫的,主代理也不模仿示例,模仿是 `voice_critic` 與「文風指令」共同負責的事。留著只會消耗 token 與汙染主代理判斷。 |
| **世界書佔位符**(如果預設裡有顯式 worldInfo 拼接節點) | 世界書已經被 ST 主流程注入到主代理拿到的訊息裡,而且 `lorebook_scout` 也能按需主動查 —— 再多一層佔位符只是冗餘。 |

### 建議保留

| 提示詞項 | 為什麼留 |
|---|---|
| **聊天曆史** | 主代理判斷「這一回合要推進什麼、誰在說話、上一拍發生了什麼」的唯一來源。一定要留,而且最好是完整的近 N 輪,不要被某些預設裡截太短的設定吃掉。 |
| **文風指令** | 直接作用於最終正文質感。主代理在 `write_message` / 給 `voice_critic` 起草任務簡報時都需要這條作為依據。 |
| **越獄 / 解除拘束指令** | 跟主對話上同等重要——主代理一旦被審查策略堵住,後面整條「偵察→起草→評審」鏈路都跑不到 finalize。 |
| **反 cliché 指令**(中文圈慣稱「反八股」) | 跟文風指令同源,主代理起草與 critic 評審都會讀它。 |
| **JSON / 函式呼叫支援** | Director 完全建立在 Chat Completion 的工具呼叫上,這一塊**必須開啟**,關掉的話整個模式跑不起來。 |

簡而言之:**只保留「關於怎麼寫」與「關於工具呼叫」的提示詞,把「角色 / 使用者 / 世界設定」這一類靜態佔位符全部交給主流程注入。**
...
::: info 跟 spec / agenda / loop 的差異
Director 沒有獨立的「匯出 profile / 匯入 profile」按鈕——跨電腦同步先用 [AI 迭代工作台](/zh-TW/features/orchestrator/iteration-studio)複用工作流。檔案級匯入匯出會等後續。
:::
