...
四項關閉的原因都是同一條:ST 主流程已經把這些內容注入到主代理看到的聊天上下文裡,預設佔位符再注一份就是重複注入。

| 提示詞項 | 為什麼關 |
|---|---|
| **角色卡欄位**(description / personality / scenario / first message / example messages) | 重複注入——已在聊天上下文裡。 |
| **使用者人設**(persona) | 重複注入——已在聊天上下文裡。 |
| **示例對話**(example messages) | 重複注入——已在聊天上下文裡。 |
| **世界書佔位符**(預設裡的顯式 worldInfo 拼接節點) | 重複注入——ST 主流程已經把世界書條目注入。 |

### 建議保留

主代理用這些內容給子代理寫任務簡報、給 critic 寫任務簡報、自己起草正文。少了任一項,主代理就沒有可參照的寫作目標。

| 提示詞項 | 為什麼留 |
|---|---|
| **聊天歷史** | 主代理唯一的「這一回合發生了什麼」窗口,要靠它給偵察 / critic 寫任務簡報、起草正文。保持完整,別被截斷設定吃掉。 |
...
| **JSON / 函式呼叫支援** | Director 完全建立在 Chat Completion 工具呼叫上,**必須開啟**,關掉整個模式跑不起來。 |

簡而言之:**只保留主代理寫任務簡報和起草正文要用的項;角色 / 使用者 / 世界設定類已經在聊天上下文裡,全部交給主流程,不要重複注入。**
