...
一個技能是一個目錄：一份必需檔案（`SKILL.md`）+ 可選子資料夾。創作一個技能就是給它取名字、寫 frontmatter、寫一段 agent 能讀的正文。僅此而已。

本頁講 Luker（與 Anthropic）期望的約定，沿途要做的選擇，以及拿兩個出廠技能逐一拆解的真實例子。
...
| **自己手寫** | [技能管理面板 → 新建](/zh-TW/features/skills/management#tab-1-installed-已安裝) | 你已經清楚知道想要哪些段落、想自己定措辭、想完全控制文字。 |

![內嵌技能編輯器 — 檔案樹 + 正文區](/_screenshots/skills/rp-demo-8.5-editor-body-pasted.png)
...
只有 `SKILL.md` 是必需的。其它資料夾是約定 —— Luker 不強制，但和 Anthropic 佈局一致，技能保持可移植。
...
| `description` | 是 | 一句話 | 顯示在自動注入的 `<available_skills>` 目錄裡。寫給 **agent** 看，不是給使用者看 —— 解釋這個技能什麼時候相關。 |
...
借鑒出廠技能的寫法，一個有效的範式有四個段落：

1. **這個技能幹什麼** —— 一段話，讓 agent 讀完就能確認選對了。
...
技能是給 LLM 看的，不是給人類維運手冊的讀者看的。兩條推論：

- **要命令式，不要描述式。** 「避免 X」比「X 通常被認為不太合適」更有效。出廠技能就是這種語氣。
- **不要元解釋。** 一段以「這是一個很棒的技能，將幫助你……」開頭的正文是浪費 token。agent 都已經決定讀它了，直接告訴它該幹什麼。
...
大多數出廠技能在 2–8 KB Markdown 之間。如果你的技能逼近 100 KB，考慮拆分：在 `SKILL.md` 放 5–15 KB 的命令式正文，把長清單挪到 `references/<topic>.md`。agent 僅在相關時才選擇性地 `skill_read({ name, path: "references/..." })` —— 更便宜的上下文、更快的讀取。
...
- **它們是二進位。** 圖片、字型、PDF 放 `assets/`。（技能裡的二進位檔案能存得下，但 agent 沒法透過 `skill_read` 讀出來。如果部署面暴露了 URL，可以作為附件 URL 使用。）
...
這個出廠技能是單檔案、約 3 KB 的 SKILL.md，沒有子檔案。它在整個預設 director profile 內強制反八股寫作規則。
...
這個技能是模式級可見：每個預設 director 子代理（主代理、口吻評審、連貫性評審、劇情頭腦風暴員……）都看得到。它錨定了整支團隊的寫作標準。

## 真實例子 2：`event-summary-rules-zh`

這個出廠技能是更密的 ~12 KB 的 SKILL.md，同樣沒有子檔案。它是 `memory_curator` 子代理的寫作契約 —— V10 事件摘要紀律。
...
這個技能是按子代理一對一：只有 `memory_curator` 在它的 `skills.visible` 裡有這一條（透過 `"+"` 繼承模式級預設值）。口吻評審不需要、連貫性評審不需要；只有 curator 需要，並且只在它真的在寫摘要時才需要。
...
- **`preset`** —— 該技能僅在搭配特定 Chat Completion 預設時才有意義（一種模型特調的提示約定、依賴取樣器的文風規則）。匯出時跟著預設走。
- **`character`** —— 該技能編碼了角色卡專屬設定或口吻規則。匯出時跟著角色卡走（PNG 元資料）。

作用域可以隨時改 —— 技能管理子面板的**遷移到……**會做原子級 move。編排器 profile 裡的參照（僅按名字）在遷移後仍然有效。
...
- `name` 必須與目錄名一致。
- 技能內部的檔案路徑不得逃出技能目錄（不能 `..`，不能絕對路徑）。
- 大小限額（見上）快速失敗強制。
- `scripts/` 會被解析但**永不執行** —— 前向相容預留。UI 在技能攜帶 scripts 時顯示風險標記。
...
- [技能管理](/zh-TW/features/skills/management) —— 安裝、匯入、匯出、遷移作用域
- [編排器整合](/zh-TW/features/skills/orchestrator-integration) —— 把技能掛到 profile 上
- [技能擴充套件 API](/zh-TW/development/extension-api/skills) —— 程式設計式 `context.skills.*`
