...
本頁講技能如何掛到編排器 profile 上、以及每個 agent 派遣時如何看到對的那一組。
...
這對需要從模式級寫作規則裡隔離出去的 agent 有用 —— 例如，只產出結構化資料的 agent 不應該看到文字評審類技能。

### deny 永遠是聯集

`deny` 跨模式 + agent 兩層取聯集（永不被替換），並且**永遠勝過** `visible`。某個技能名字只要出現在任一 deny 清單裡，即使也在某個 visible 清單裡，仍然被過濾掉。
...
`visible` 接受 `"*"`，意為「所有已安裝技能」。早期想讓 agent 看到所有可用項又不想列名時很有用：
...
（用 skill_read 檢視具體內容；用 skill_search 在一個技能內 grep。）
```
...
被限制在可見集內，這是 agent 唯一能拿到技能內容的方式：
...
| `skill_search({ name, query, path?, limit?, contextLines? })` | 在單個技能的檔案內做子字串搜尋 | `{ hits: [{ path, lineStart, lineEnd, snippet }] }` |

agent 真的需要正文時就呼叫 `skill_read`。`skill_search` 用於技能很大、只有某一段相關的場景。

執行時硬上限：`skill_read` 響應不超過 50 KB。截斷時 agent 用 `offset` 繼續讀。這保證即使技能攜帶大塊參考檔案，上下文成本也是有界的。
...
- **每個子代理額外拿到自己的方法技能** —— `voice_critic` 讀 `voice-critic-method-zh`，`memory_curator` 讀 `event-summary-rules-zh`，依此類推。
...
::: info 審查節點跳過目錄注入
spec 模式的審查節點（review nodes）的目錄注入是有意跳過的 —— 審查是結構化的判斷任務而不是內容生成任務，給它的 prompt 加 `<available_skills>` 只是雜訊。審查節點仍然可以顯式呼叫 `skill_list` 或 `skill_read` 看到技能，但自動注入的目錄被省略。
:::

## 失效參照

`skills.visible` 是名字清單 —— 每個技能在派遣時才校驗物理存在。

- **缺失的技能**（改名 / 刪除 / 未安裝）：靜默過濾掉。agent 看不到；沒有錯誤。
- **在編輯器 UI 裡**：該條目變灰，帶 tooltip 「該技能未安裝」。

這種「軟失敗」行為是有意的。意味著：

- 匯入一張你跳過其內嵌技能的角色卡不會壞任何東西 —— 參照只是懸空。
- 刪除一個技能不需要清理每一個參照它的 profile。
- 改名一個技能是一步操作 —— 參照變失效，但派遣繼續工作，直到你選擇修。

## 編輯策略

在編排器面板裡，每個 agent 的設定卡顯示一行 **技能**：

![每 agent 的技能 chip，帶 + 繼承標記](/_screenshots/skills/agent-skill-chips.png)

- **`+`** chip —— 顯式的「繼承模式預設值」標記（`visible` 以 `"+"` 開頭時出現）。
- 每個具名技能一個 chip。點擊移除；點 **新增……** 從你已安裝的庫存裡挑。
- **禁用** 行 —— 同樣的 chip，獨立清單。

面板頂部的模式級 **技能** 行也一樣。編輯它更新 `mode.skills`；編輯某個 agent 的行更新該 agent 的覆寫。

要讓 AI 幫你編輯，[迭代工作台](/zh-TW/features/orchestrator/iteration-studio) 自帶技能繫結工具（`skill_bind_to_agent`、`skill_unbind_from_agent`、`skill_set_mode_defaults`）—— 用自然語言描述需求，AI 透過工具呼叫 patch 你的策略。

## 模式切換會失效

編排器從一種模式切到另一種（例如 director → loop）時，執行時把快取的解析結果作廢。下一次派遣按新模式的策略重新建構目錄。UI 也按新模式的 profile 重新渲染 **技能** 行，所以編輯永遠反映當前激活的模式。

意味著 `director.mainAgent.skills` 區塊切到 loop 模式後不會跟過去。每種模式自己一份策略。許多使用者在多種模式之間保持類似形狀（同樣的共享技能、同樣的 per-agent 專屬），但編輯是按模式獨立的。

## 相關

- [技能概覽](/zh-TW/features/skills/) —— 什麼是技能、三種作用域
- [創作技能](/zh-TW/features/skills/authoring) —— 寫自己的
- [技能管理](/zh-TW/features/skills/management) —— 安裝 / 遷移 / 刪除
- [Director 模式](/zh-TW/features/orchestrator/director) —— 出廠自帶完整技能整合的典型多 agent profile
- [AI 迭代工作台](/zh-TW/features/orchestrator/iteration-studio) —— 用自然語言 AI 編輯 patch 策略
- [技能擴充套件 API](/zh-TW/development/extension-api/skills) —— 從擴充套件程式設計式編輯策略
