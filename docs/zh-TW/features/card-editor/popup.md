...
當你透過替換或更新操作匯入新的角色卡時，如果新卡自帶內建世界書（或新舊卡繫結了不同的世界書），編輯助手會彈出世界書同步彈窗，提供三種處理方式：

![世界書同步彈窗：三種處理方式](/images/card-editor-popup/cea-lorebook-sync.png)

- **匯入新世界書** — 把新卡自帶的內建世界書另存為獨立檔案並繫結給這個角色。當你想原樣使用新卡自帶的設定時選它。
- **保留舊世界書** — 重新繫結回之前那本世界書，忽略新卡自帶的內建世界書。當你只想更新角色卡欄位時選它。
- **在編輯器中合併** — 開啟迭代工作台，帶上「舊 vs 新」的差異，讓 AI 把你之前手動補充的內容遷移到新書中。當你有手動整理過、不想遺失的內容時選它。

點「取消」則什麼都不動。

選擇「在編輯器中合併」後，工作台頂欄還會有一個「檢視完整替換差異」按鈕，點開是一個全螢幕結構化差異總覽：把你之前使用的角色卡 + 世界書 vs 剛剛匯入的這份，按欄位和條目分卡片列出來，附逐行 diff 和詞級紅綠高亮。呈現給你的資訊和 AI 在首輪拿到的簡報是同一份：

![完整替換差異彈窗——舊版本 vs 新版本的角色卡和世界書](/images/card-editor-popup/cea-replace-diff.png)

如果你在工作台裡沒有套用任何 AI 提出的修改就直接關掉，之前的預匯入（新世界書檔案 + 角色繫結）會自動回滾，取消一次合併會話等同於替換從未發生過。
