...
- **edits-lib usage now has two documented pathways**: Path 1 (iteration-studio shell adapter) for popup-friendly surfaces, Path 2 (library-only) for fullscreen / custom-UI. See `docs/development/extension-api/edits-lib.md` "Path 2: library-only". CardApp Studio is the in-tree Path 2 reference.
