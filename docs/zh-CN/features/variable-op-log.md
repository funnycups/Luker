...
::: info JSON 形态的 value
value 末尾是字面 `}` 的情形（典型 `{"x":1}` / `[1,2]` 类负载）会让宏末尾出现连续三个 `}`——一个是 JSON 闭合，两个是宏 close。扫描器把任意末尾连续 `}` 里**最后一对** `}}` 当作宏 close，所以 `{{setvar::config::{"x":1}}}` 不需要转义就能正确解析。

副作用：宏紧跟字面 `}` 写在叙事里（例如 `... {{macro}}}`）会把那个 `}` 吞进 value。这种场景请在中间加空格（`{{macro}} }`）保持分离。
:::
