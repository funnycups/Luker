...
一个技能是一个目录：一份必需文件（`SKILL.md`）+ 可选子文件夹。创作一个技能就是给它取名字、写 frontmatter、写一段 agent 能读的正文。仅此而已。

本页讲 Luker（与 Anthropic）期望的约定，沿途要做的选择，以及拿两个出厂技能逐一拆解的真实例子。
...
| **自己手写** | [技能管理面板 → 新建](/zh-CN/features/skills/management#tab-1-installed-已安装) | 你已经清楚知道想要哪些段落、想自己定措辞、想完全控制文字。 |
...
只有 `SKILL.md` 是必需的。其它文件夹是约定 —— Luker 不强制，但和 Anthropic 布局一致，技能保持可移植。
...
| `description` | 是 | 一句话 | 显示在自动注入的 `<available_skills>` 目录里。写给 **agent** 看，不是给用户看 —— 解释这个技能什么时候相关。 |
...
借鉴出厂技能的写法，一个有效的范式有四个段落：

1. **这个技能干什么** —— 一段话，让 agent 读完就能确认选对了。
...
技能是给 LLM 看的，不是给人类运维手册的读者看的。两条推论：

- **要命令式，不要描述式。** 「避免 X」比「X 通常被认为不太合适」更有效。出厂技能就是这种语气。
- **不要元解释。** 一段以「这是一个很棒的技能，将帮助你……」开头的正文是浪费 token。agent 都已经决定读它了，直接告诉它该干什么。
...
大多数出厂技能在 2–8 KB Markdown 之间。如果你的技能逼近 100 KB，考虑拆分：在 `SKILL.md` 放 5–15 KB 的命令式正文，把长清单挪到 `references/<topic>.md`。agent 仅在相关时才选择性地 `skill_read({ name, path: "references/..." })` —— 更便宜的上下文、更快的读取。
...
- **它们是二进制。** 图片、字体、PDF 放 `assets/`。（技能里的二进制文件能存得下，但 agent 没法通过 `skill_read` 读出来。如果部署面暴露了 URL，可以作为附件 URL 使用。）
...
这个出厂技能是单文件、约 3 KB 的 SKILL.md，没有子文件。它在整个默认 director profile 内强制反八股写作规则。
...
这个技能是模式级可见：每个默认 director 子代理（主代理、口吻评审、连贯性评审、剧情头脑风暴员……）都看得到。它锚定了整支团队的写作标准。

## 真实例子 2：`event-summary-rules-zh`

这个出厂技能是更密的 ~12 KB 的 SKILL.md，同样没有子文件。它是 `memory_curator` 子代理的写作契约 —— V10 事件摘要纪律。
...
这个技能是按子代理一对一：只有 `memory_curator` 在它的 `skills.visible` 里有这一条（通过 `"+"` 继承模式级默认值）。口吻评审不需要、连贯性评审不需要；只有 curator 需要，并且只在它真的在写摘要时才需要。
...
- **`preset`** —— 该技能仅在搭配特定 Chat Completion 预设时才有意义（一种模型特调的提示约定、依赖采样器的文风规则）。导出时跟着预设走。
- **`character`** —— 该技能编码了角色卡专属设定或口吻规则。导出时跟着角色卡走（PNG 元数据）。

作用域可以随时改 —— 技能管理子面板的**迁移到……**会做原子级 move。编排器 profile 里的引用（仅按名字）在迁移后仍然有效。
...
- `name` 必须与目录名一致。
- 技能内部的文件路径不得逃出技能目录（不能 `..`，不能绝对路径）。
- 大小限额（见上）快速失败强制。
- `scripts/` 会被解析但**永不执行** —— 前向兼容预留。UI 在技能携带 scripts 时显示风险标记。
...
- [技能管理](/zh-CN/features/skills/management) —— 安装、导入、导出、迁移作用域
- [编排器集成](/zh-CN/features/skills/orchestrator-integration) —— 把技能挂到 profile 上
- [技能扩展 API](/zh-CN/development/extension-api/skills) —— 编程式 `context.skills.*`
