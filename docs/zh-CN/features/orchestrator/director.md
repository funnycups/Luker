...
4. **默认 profile 自带 8 个为 RP 优化过的子代理**:
   - 前置侦察(可并行派,各扫一个源):`chat_scout`(扫聊天)、`memory_scout`(扫记忆图)、`lorebook_scout`(扫世界书)。
   - 跨源前置侦察:`epistemic_scout` 把聊天(角色经历过什么)跟世界书 / 记忆图(世界里本可被知道的事)交叉对位,产出每个在场角色的「知道 / 不知道 / 上帝视角陷阱」清单——避免笔下角色不小心展示了 ta 在故事内不可能拥有的认知。
   - 按需外部侦察:`canon_scout` —— 仅当本回合涉及二次创作 / 公共 IP 且需要核对原典时派,原创故事不要派。
   - 中段头脑风暴:`plot_brainstormer` —— 各按一个具体角度产出一份下一拍的结构草图;并行派多个不同角度的拿到真正不同的选项。
   - 后置评审(草稿后可并行派):`voice_critic`(口吻 / 角色一致性)、`continuity_critic`(连续性)。
