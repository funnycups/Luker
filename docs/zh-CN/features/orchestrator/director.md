...
默认主代理系统提示词与默认的八个子代理 id **强耦合**——它已经按「先派侦察、起草、再派评审、迭代修订」的纪律调好了。要改的话推荐用 [AI 迭代工作台](/zh-CN/features/orchestrator/iteration-studio)用自然语言描述需求,让它通过工具调用 patch 你的 profile。
:::
...
四项关闭的原因都是同一条:ST 主流程已经把这些内容注入到主代理看到的聊天上下文里,预设占位符再注一份就是重复注入。

| 提示词项 | 为什么关 |
|---|---|
| **角色卡字段**(description / personality / scenario / first message / example messages) | 重复注入——已在聊天上下文里。 |
| **用户人设**(persona) | 重复注入——已在聊天上下文里。 |
| **示例对话**(example messages) | 重复注入——已在聊天上下文里。 |
| **世界书占位符**(预设里的显式 worldInfo 拼接节点) | 重复注入——ST 主流程已经把世界书条目注入。 |

### 建议保留

主代理用这些内容给子代理写任务简报、给 critic 写任务简报、自己起草正文。少了任意一项,主代理就没有可参照的写作目标。

| 提示词项 | 为什么留 |
|---|---|
| **聊天历史** | 主代理唯一的「这一回合发生了什么」窗口,要靠它给侦察 / critic 写任务简报、起草正文。保持完整,别被截断设置吃掉。 |
...
| **JSON / 函数调用支持** | Director 完全建立在 Chat Completion 工具调用上,**必须开启**,关掉整个模式跑不起来。 |

简而言之:**只保留主代理写任务简报和起草正文要用的项;角色 / 用户 / 世界设定类已经在聊天上下文里,全部交给主流程,不要重复注入。**
