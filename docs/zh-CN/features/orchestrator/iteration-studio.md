...
在角色卡专属编排会话里打开工作台时，AI 还会顺便对照这张卡绑定的世界书。如果某条条目硬性约束了输出（强制固定格式、要求纯文本、禁用 markdown、固定回合结构），并且这种约束和你正在搭建的编排的输出契约冲突，工作台会就地调整这条条目——通常是直接禁用（保留正文，方便以后重新启用），少数情况会改写几句正文以放宽约束。每次调整都会和编排提案一起出现在会话历史里。
