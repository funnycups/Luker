...
聊天加载时 memory-graph 会跑一遍迁移 pipeline，把旧的持久化形态（v5 raw、
v8 opLog）翻译到当前的 v2 floor-state 布局（graph payload + `__floor_log`
提交日志 + `__meta` sidecar）。pipeline 幂等，只在输入形态不是 v2 时才改
chat-state，任意一步失败均不改写。
