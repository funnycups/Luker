...
- 一份能用的 [Chat Completion 预设](/zh-CN/basics/presets)，最好已经调教过文风、越狱、NSFW 指导

## Step 1 — 挑一份起点预设

任何你日常用的 RP 预设都行。这一步只是确认你有一份调教好的写作预设作为起点——下一步要从它派生一份给 Agent 用的变种。

::: tip 为什么不直接用日常预设跑 Agent
日常 RP 预设里大量提示词假设「主 LLM 一个人独自写完整个回复」，这些指令塞进 Agent 工具循环里会：

- 跟 Agent 的 system prompt 抢话语权
- 把「必须输出 schema」「强制思维链」这类格式约束塞进起草环节，顶坏 tool calling
- 让占位符（角色卡描述、人设、世界书条目）被重复注入

派生一份是最安全的——原预设保留不动，你随时可以切回去。
:::

## Step 2 — 派生一份「Agent 专用」预设

打开**聊天补全预设助手**，工具栏顶部把**编辑模式**切到「**编排器适配**」，然后跟它说：
...
::: tip 顺手让它再帮你改造预设
「编排器适配」只是助手三种**编辑模式**里的一种。把工具栏的**编辑模式**切回默认的「**通用编辑**」开一个新会话，同一个助手就是一个通用预设编辑器——可以让它「添加一个反八股指导并补充反面示例」「把文风指导从浓墨重彩改成克制细腻」「合并几条意思重复的规则」之类。详见[补全预设助手](/zh-CN/features/preset-assistant)。
:::

## Step 3 — 切到 Director 模式，配好两路 API

打开扩展抽屉的**多智能体编排**面板：

1. **执行模式** 切到 **Director（多代理）**
2. **API 预设** + **提示词预设** 选 Step 2 派生的那份 `-orchestrator` 预设
3. 找到 **AI 迭代工作台** 区域，把它的 **API 预设** + **提示词预设** 也填一份——但这里**不要**用 `-orchestrator`，用一份**精简到只剩越狱**的预设（本文档附带的 [`plugin-only.json`](/presets/plugin-only.json) 就是为这种场景准备的）

::: tip 两路预设分工
| 路径 | 给谁用 | 选什么 |
|---|---|---|
| **Agent API + 提示词预设** | 编排器里实际跑的主 Agent + 子 Agent，直接产出正文 | Step 2 派生的 `-orchestrator` 预设（自带越狱、NSFW、文风指导） |
| **迭代工作台 API + 提示词预设** | 你在工作台里跟 AI 对话调编排时，工作台 AI 自己用 | `plugin-only.json` 或类似的纯净预设——它的工作是修改你的编排配置，不需要文风、越狱 |

Luker 内置的其他插件（记忆图 Schema 工作台、CardApp Studio、预设助手自身……）的「迭代 AI」也都建议挂这份精简预设。
:::
...
记忆图面板里的 **Schema 迭代提示词（schema-editor AI）** 那一栏走的就是 Step 3 提到的「迭代 AI 路径」——这里也建议挂 `plugin-only.json` 这样的精简预设。
:::
...
想看更细——每个 Agent 的具体模型思考、每次工具调用的请求和响应？编排器面板里点 **查看运行态轨迹**。
...
- [补全预设助手](/zh-CN/features/preset-assistant) — 「编排器适配」之外的两种会话模式
