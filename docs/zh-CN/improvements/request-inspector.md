...
请求记录保存在内存中，每个用户最多保留 200 条。每个用户只保留最近的记录；达到上限后，最早的记录会被移除腾出空间。

已完成的记录保留两小时，之后移除。仍在生成中的记录不受此时限约束：进行中的请求最长保留六小时，长时间生成或停滞的流不会在完成回调执行前被清理。过期记录每五分钟清扫一次，因此一条记录最多在发起后保留两小时零五分钟。

两个数值都可以在 `config.yaml` 中调整：
...
- `ttlMs`：已完成记录的保留时长（毫秒）
- `cleanupIntervalMs`：过期清扫的执行间隔（毫秒）
...
> [!TIP]
> 请求检查器会随服务器自动启用，默认值开箱即用；需要调整时再改上面的保留设置即可。
