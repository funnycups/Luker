...
`context.skills.*` 是用于安装、读取、编辑、打包技能的 JavaScript 接口。扩展通过 `Luker.getContext()` 拿到它；CardApp 在自己的 `ctx.skills` 上拿到同样的形状。

技能是编排器使用的[知识包](/zh-CN/features/skills/)；这套 API 是支撑技能管理子面板、内嵌编辑器、迭代工作台 17 个技能工具的读写传输层。

::: tip 请先读用户文档
关于概念模型 —— 作用域、可见性策略、嵌入生命周期 —— 从 [技能概览](/zh-CN/features/skills/) 开始。本页是 API 参考。
:::
...
| `fileCount` | `number` | 技能目录里的文件总数。 |
| `totalBytes` | `number` | 文件大小总和。 |
| `hasScripts` | `boolean` | 技能携带 `scripts/` 目录时为 true。 |
...
省略 `scope` 时返回首个匹配 —— 适合用作「这个技能某处存在吗」的检查，但不能替代编排器的后者优先解析。需要确定性查找时传入特定 scope。
...
`SKILL.md` 永远在第一位；其余按 `localeCompare` 排序。在不清楚技能里有什么时，先用这个再 `readFile`。
...
删除技能内的单个文件。`SKILL.md` 不能通过这条路径删 —— 用 `delete(scope, name)` 删除整个技能。
...
原子移除技能目录。引用该技能的编排器 profile 仍保留 `skills.visible` 里的旧名 —— 派遣时引用软失败。
...
跨作用域原子文件系统 move。编排器引用（只按名字）依然有效。想要覆盖目标，先删目标作用域里的同名技能再 move 即可。

### `importBundled()`

从 `default/skills/global/` 重装所有出厂技能，覆盖任何同名本地副本。

```ts
importBundled(): Promise<{
  installed: number,   // 处理的出厂技能总数
...
::: warning 破坏性
这是技能版的 `git reset --hard` —— 它把每个同名全局技能都覆盖成出厂版本。先备份本地修改。
:::

### `listBundledManifest()`

列出 `default/skills/global/` 下出厂的技能，每个都附带导入后会生成的 install 哈希。**浏览出厂** tab 用这个来跟本地安装对比，无需重跑 install。
...
这些函数支持通过角色卡和预设跨主机分发技能。

### `packForEmbed(opts)`

把一个或多个技能打包成嵌入载荷，可塞进预设或角色卡。
...
`mode: 'auto'` 时打包器按每技能自选：小且纯文本的用内联，更大或二进制的用存档。
...
`conflictStrategies` 按技能名为 key；缺失条目缺省按 `'different'` 抛错。`'same'` 条目永远静默 no-op。

### `importFromUrl(opts)`

从 HTTPS URL 拉取单个 `SKILL.md` 并作为单文件技能安装。
...
多文件技能不能通过 URL 导入 —— 用 `install()` 配 `archive-base64-v1` 载荷（或技能管理的**从文件导入**流程）。
...
- [创作技能](/zh-CN/features/skills/authoring) —— frontmatter + 正文约定
- [技能管理](/zh-CN/features/skills/management) —— 同等操作的 UI 接口
