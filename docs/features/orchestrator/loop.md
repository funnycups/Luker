...
| `memory_rank(query, mode?, types?, k?)` | Rank candidates by `recency` / `vector` / `keyword` / `hybrid`. Returns `{ ranked: [{ id, type, title, seqTo, score, scoreMode }] }`. Use when the candidate pool is large and you need a fast first-pass shortlist before fetching briefs. | `memory_rank({ query: 'family secret', mode: 'hybrid', k: 8 })` surfaces the 8 most relevant nodes by hybrid score. |
| `memory_node_brief(node_id, include_edge_summary?, edge_summary_limit?)` | Canonical brief for one node (title, summary, key/row values, child count, exposure, edge summary, alwaysInject flag) — the same per-row format the recall LLM sees. | After a rank shortlist, `memory_node_brief({ node_id: 'evt_42' })` fetches the full brief for one node. |
