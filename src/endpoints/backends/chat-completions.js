...
import {
    ...
    forwardStreamingWithInspectorTap,
...
async function forwardStreamingResponseWithJob(request, response, fetchResponse) {
    ...
    return await forwardStreamingWithInspectorTap(fetchResponse, response, request);
}
...
async function sendClaudeRequest(request, response) {
    const apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_CLAUDE).toString();
    ...
    try {
        ...
        attachInspectionEndpoint(request, apiUrl + '/messages', apiKey, requestBody);
...
async function sendMakerSuiteRequest(request, response) {
    ...
    if (useVertexAi) {
        apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_VERTEX_AI);
        ...
        apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_MAKERSUITE);
    ...
    try {
        ...
        attachInspectionEndpoint(request, url, apiKey, body);
...
async function sendAI21Request(request, response) {
    ...
    const apiKey = readProviderSecret(request, SECRET_KEYS.AI21);
    if (!apiKey) {
    ...
    try {
        attachInspectionEndpoint(request, API_AI21 + '/chat/completions', apiKey, body);
...
async function sendMistralAIRequest(request, response) {
    const apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_MISTRAL).toString();
    ...
    try {
        ...
        attachInspectionEndpoint(request, apiUrl + '/chat/completions', apiKey, requestBody);
...
async function sendCohereRequest(request, response) {
    const apiKey = readProviderSecret(request, SECRET_KEYS.COHERE);
    const controller = new AbortController();
    bindRequestCloseAbort(request, controller);
    ...
    try {
        ...
        attachInspectionEndpoint(request, apiUrl, apiKey, requestBody);
        if (request.body.stream) {
...
async function sendDeepSeekRequest(request, response) {
    const apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_DEEPSEEK).toString();
...
async function sendXaiRequest(request, response) {
    const apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || API_XAI).toString();
...
async function sendAimlapiRequest(request, response) {
    ...
    const apiKey = readProviderSecret(request, SECRET_KEYS.AIMLAPI);
...
async function sendElectronHubRequest(request, response) {
    ...
    const apiKey = readProviderSecret(request, SECRET_KEYS.ELECTRONHUB);
...
async function sendChutesRequest(request, response) {
    ...
    const apiKey = readProviderSecret(request, SECRET_KEYS.CHUTES);
...
async function sendAzureOpenAIRequest(request, response) {
    ...
    try {
        attachInspectionEndpoint(request, endpointUrl, apiKey, apiRequestBody);
...
router.post('/generate', async function (request, response) {
    try {
        ...
        if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.OPENAI) {
            ...
            apiUrl = new URL(request.body.reverse_proxy || request.body.base_url || defaultApiUrl).toString();
        ...
        attachInspectionEndpoint(request, endpointUrl, apiKey, requestBody);
        const fetchResponse = await fetch(endpointUrl, config);
