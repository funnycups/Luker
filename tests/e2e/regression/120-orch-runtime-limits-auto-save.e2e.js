...
test.describe('#120 — orchestrator Runtime-limits auto-save', () => {
    ...
    test('director maxRounds change persists across reload without touching mainAgent draft', async ({ page }) => {
        ...
        // workspace (must be visible after mode switch to director).
        const mainAgentPromptBefore = await page
        ...
        // Cross-check: mainAgent systemPrompt must NOT have been persisted
        // by the auto-save — if the auto-save accidentally used the whole
        // editor draft instead of a narrow patch, the mainAgent would have
        // been flushed too. Verify by making sure the value we saw before
        // the change round-tripped unchanged (which is the pre-fix baseline).
        const mainAgentPromptAfter = await page
            .locator('[data-orch-director-field="mainAgent.systemPrompt"]')
            .first()
            .inputValue();
        expect(mainAgentPromptAfter,
            'mainAgent.systemPrompt must NOT be persisted by the narrow-patch auto-save; ' +
            'if it was, the persisted-then-hydrated value would drift from the original',
