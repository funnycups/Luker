...
class StubElement {
    ...
    // Minimal classList backed by the `class` attribute, so it stays
    // consistent with querySelectorAll's class matching. Enough for the
    // panel's add/remove/toggle/contains calls (e.g. the import menu's
    // open-state class).
    get classList() {
        const el = this;
        const read = () => String(el._attrs.get('class') || '').split(/\s+/).filter(Boolean);
        const write = (arr) => el._attrs.set('class', arr.join(' '));
        return {
            add: (c) => { const a = read(); if (!a.includes(c)) { a.push(c); write(a); } },
            remove: (c) => { write(read().filter((x) => x !== c)); },
            toggle: (c) => {
                const a = read();
                if (a.includes(c)) { write(a.filter((x) => x !== c)); return false; }
                a.push(c); write(a); return true;
            },
    ...
    matches(sel) {
        ...
        // every class is present in the element's class list. Strip
        // `[attr="value"]` brackets first so a dot inside an attribute value
        // (e.g. [data-x="C.png"]) isn't mistaken for a `.class` selector.
        ...
        const remainderNoAttrs = remainder.replace(/\[[^\]]*\]/g, '');
        while ((cm = classRe.exec(remainderNoAttrs))) {
