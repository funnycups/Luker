...
export function render(payload, i18n) {
    ...
    rounds.forEach(round => {
        ...
        const roundSec = appendShared.section(root, roundLabel, null);

        const planner = round.planner || {};
        const plannerBase = `${roundLabel} → Planner`;
        const plannerSec = appendShared.subsection(roundSec, 'Planner', null);
        renderTurns(plannerSec, planner.turns || [], plannerBase, i18n);
        const plannerOut = appendShared.subsection(plannerSec, i18n('sim.label.output', 'Output'), `${plannerBase} → Output`);
        ...
        dispatches.forEach(d => {
            ...
            const dSec = appendShared.subsection(roundSec, `Dispatch: ${d.agentName} (todo=${d.todoId})`, null);
            appendShared.note(dSec, `Task brief: ${d.taskBrief || ''}`);
            renderTurns(dSec, d.turns || [], dBase, i18n);
            const out = appendShared.subsection(dSec, i18n('sim.label.output', 'Output'), `${dBase} → Output`);
            appendShared.pre(out, d.output || '');
        });
    });

    const finalizer = payload?.finalizer || {};
    const finSec = appendShared.section(root, i18n('sim.section.finalizer', 'Finalizer'), null);
    renderTurns(finSec, finalizer.turns || [], 'Finalizer', i18n);
    const finOut = appendShared.subsection(finSec, i18n('sim.label.output', 'Output'), 'Finalizer → Output');
    appendShared.pre(finOut, finalizer.output || '');

    const composedSec = appendShared.section(root, i18n('sim.section.final_composed', 'Final Composed Output'), 'Final Composed Output');
...
function renderTurns(parent, turns, basePath, i18n) {
    turns.forEach((turn, i) => {
        ...
        const turnSec = appendShared.subsubsection(parent, turnLabel, null);
        if (turn.reasoning) {
        ...
        if (turn.assistantText) {
            const a = appendShared.subsubsection(turnSec, i18n('sim.label.assistant', 'Assistant'), `${turnPath} → Assistant`);
        ...
        tcs.forEach((tc, j) => {
            ...
            const tcSec = appendShared.subsubsection(turnSec, `Tool call #${j + 1}: ${tc.name}`, null);
