...
export function render(payload, i18n) {
    ...
    stages.forEach(stage => {
        ...
        const stageSec = appendShared.section(root, `${stageLabel} [${stage.mode}]`, null);
        const nodes = Array.isArray(stage.nodes) ? stage.nodes : [];
        nodes.forEach(node => {
            const nodePath = `${stageLabel} → Node "${node.id}"`;
            const nodeSec = appendShared.subsection(stageSec, `${node.kind} — ${node.id}`, null);
            ...
            const outSec = appendShared.subsection(nodeSec, i18n('sim.label.output', 'Output'), `${nodePath} → Output`);
            appendShared.pre(outSec, node.output || '');
        });
    });

    if (typeof payload?.finalCapsule === 'string' && payload.finalCapsule) {
        const sec = appendShared.section(root, i18n('sim.section.final_capsule', 'Final Capsule'), 'Final Capsule');
...
function renderTurns(parent, turns, basePath, i18n) {
    turns.forEach((turn, i) => {
        ...
        if (turn.assistantText) {
            const asec = appendShared.subsubsection(turnSec, i18n('sim.label.assistant', 'Assistant'), `${turnPath} → Assistant`);
