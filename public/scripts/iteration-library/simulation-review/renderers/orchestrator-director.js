...
export function render(payload, i18n) {
    ...
    const mainSec = appendShared.section(root, i18n('sim.section.main_agent', 'Main Agent'), null);
    ...
    rounds.forEach(round => {
        ...
        const sec = appendShared.subsection(mainSec, roundLabel, null);
        if (round.reasoning) {
        ...
        if (round.assistantText) {
            const a = appendShared.subsubsection(sec, i18n('sim.label.assistant', 'Assistant'), `${base} → Assistant`);
        ...
        tcs.forEach((tc, j) => {
            ...
            const tcSec = appendShared.subsubsection(sec, `Tool call #${j + 1}: ${tc.name}`, null);
    ...
    subs.forEach(sub => {
        ...
        const sec = appendShared.section(root, `Sub-agent: ${sub.subagentId}${sub.isInline ? ' (inline)' : ''}`, null);
        ...
        const out = appendShared.subsection(sec, i18n('sim.label.output', 'Output'), `${subBase} → Output`);
        appendShared.pre(out, sub.output || '');
    });

    const finSec = appendShared.section(root, i18n('sim.section.final_message', 'Final Message'), 'Final Message');
