...
export function render(payload, i18n) {
    ...
    const out = appendShared.section(root, i18n('sim.section.final_output', 'Final Output'), 'Final Output');
    ...
    if (payload?.assembledPrompt) {
        ...
        if (payload.assembledPrompt?.systemPrompt) {
            const sysSec = appendShared.subsection(ap, i18n('sim.section.assembled_system', 'System'), 'Assembled Prompt → System');
        ...
        messages.forEach((m) => {
            ...
            const sec = appendShared.subsection(ap, label, path);
