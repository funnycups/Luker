...
export function render(payload, i18n) {
    ...
    rounds.forEach(round => {
        ...
        const sec = appendShared.section(root, roundLabel, null);
        if (round.reasoning) {
        ...
        if (round.assistantText) {
            const a = appendShared.subsection(sec, i18n('sim.label.assistant', 'Assistant'), `${roundLabel} → Assistant`);
        ...
        tcs.forEach((tc, j) => {
            ...
            const tcSec = appendShared.subsection(sec, `Tool call #${j + 1}: ${tc.name}`, null);
    ...
    if (payload?.capsule) {
        const sec = appendShared.section(root, i18n('sim.section.capsule', 'Capsule'), 'Capsule');
