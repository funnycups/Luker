...
const DEFAULT_EVENT_COMPRESSION_INSTRUCTION = [
    ...
    '',
    '## Rollup-only gates (in addition to the shared writing standard above)',
    '',
    '### Gate — 24-hour persistence test (compression layer only)',
    'For every clause you draft, ask: 这件事在剧情时间往后推 24 小时之后，是否仍对世界状态构成约束？',
    'KEEP — 会过夜的事实:',
    '  - 关系 / 隶属 / 契约 / 阵营 的建立、变更、破裂',
    '  - 知识 / 情报 / 真相 的获得、暴露、丢失',
    '  - 角色 / 势力的死伤、阵亡、瓦解、新生、晋升',
    '  - 物品 / 钥匙 / 通行权 / 资源 的获得或转让',
    '  - 不可逆的物理状态变更；地点的封印 / 解锁；剧情段的开启 / 关闭',
    'CUT — 仅当下成立 (these belong on the leaf children, not on the rollup):',
    '  - 此刻的姿势、位置、衣物、装饰、表情、神态、嗓音、微表情',
    '  - 一次互动里的具体动作节奏、机械步骤、身体细节、感官描写',
    '  - 单次相遇的具体字词与口吻 (即便没用引号，复述其大意也属违规)',
    '  - 一次性的环境描写 (陈设 / 灯光 / 气味 / 摆盘 / 天气片段)',
    'Worked example A (任意题材):',
    '  ✗ A 在月光花园里单膝跪地、声音颤抖地向 B 求婚并获允',
    '  ✓ A 向 B 求婚并获允',
    '  Reason: 婚约 24h 后仍约束；花园 / 跪地 / 颤抖嗓音 24h 后无残留 — those details live on the leaf.',
    'Worked example B (任意题材):',
    '  ✗ A 横扫长剑、单膝跪地完成必杀技，将首领头颅斩落',
    '  ✓ A 击杀首领',
    '  Reason: 击杀 24h 后仍约束；姿势与招式细节属当下表演 — those details live on the leaf.',
    'Worked example C (任意题材):',
    '  ✗ A 反复试探、欲拒还迎地诱使 B 吐露秘密未果',
    '  ✓ A 胁迫 B 吐露秘密，未果',
    '  Reason: "未果" 24h 后仍约束；试探 / 欲拒还迎是单次互动节奏。',
    '',
    '### Gate — Density budget (compression layer only)',
    'Honor the per-layer budget given in the user message; when unset, default to:',
    '  - L1 rollup: each child ≤ 2 主谓短语; total chars ≤ fanIn × 50 soft / fanIn × 80 hard.',
    '  - L2-L3 rollup: each child ≤ 1 主谓短语; total ≤ fanIn × 30.',
    '  - L4+ rollup: only stage-level changes (势力 / 关系 / 世界状态 / 未解钩子的转移); ≤ 3 并列点; no per-child enumeration.',
    'Never concatenate or copy-paste child summaries — re-synthesize.',
    '',
    '## Required <thought> before tool call',
    'You must output exactly one <thought>...</thought> block before calling the summary tool.',
    'The <thought> block must contain sections [1]-[5] in order. Reference children by their id as listed in the user message.',
    '',
    ...
    '  <一段>',
    '',
    '[5] 写作合规自检（必须按 [5a] → [5b] → [5c] 顺序，禁止跳过 [5a]；只打 ✓ 不列条目视为未做，必须返工）：',
    '',
    '[5a] 候选物清点（先列，不判断；空类目必须写"无"，不得省略行）：',
    '  - 引号字符串：枚举 [4] 中所有 " / "  / “ ” / ‘ ’ 包裹的内容（含其前后约 5 字上下文）',
    '  - 主谓短语：把 [4] 拆成编号短语列表（每行一个），后续判定逐编号引用',
    '  - 姿势/位置/衣物/装饰：列出所有命中此类语义的名词短语',
    '  - 表情/神态/嗓音/微表情：列出所有命中此类语义的短语',
    '  - 肢体节奏/机械步骤/感官细节：列出所有描述单次互动中身体节奏或感官刺激的短语',
    '  - 续写/伏笔/未来时：列出"暗示" / "为X埋下伏笔" / "后续将" / "钩子" 等出现',
    '',
    '[5b] 判定（针对 [5a] 每个条目，逐条引用编号或字面，写判定 + 一句理由）：',
    '  零引号闸门：[5a] 引号清点的每个条目均违规——若是真专有名词则去引号，否则去引号并改写为动作描述。',
    '  24h 持久性闸门：对 [5a] 主谓短语逐编号做 24h 测试，标 KEEP / CUT + 一句理由；',
    '    [5a] 的姿势 / 表情 / 肢体节奏 / 感官细节清点条目一律 CUT，无 24h 豁免（这些细节已保留在叶层，不需要在 rollup 复述）。',
    '  密度闸门：写出 fanIn = N、保留主谓短语数 = M、改写后预估字数 = K；与 [3] 的层级预算对照，超额需再压。',
    '  续写闸门：[5a] 续写词清点的每个条目均删除。',
    '  照搬闸门：抽样估算 [4] 与 [1] 子节点点检的字面重合率；≥ 50% 视为照搬，必须重写。',
    '',
    '[5c] 改写后草稿（仅当 [5b] 出现任意违规时必填，否则写"[4 通过，无变化]"）：',
    '  按 [5b] 判定结果重写 [4]，给出最终成稿。',
    '',
    'After </thought>, call the summary tool. The summary field must equal the [5c] result word-for-word; if [5c] is "[4 通过，无变化]", use [4] verbatim.',
