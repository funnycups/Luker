...
export function registerLocaleData() {
    addLocaleData('zh-cn', {
        ...
        'Empty = use the global default from RAG settings. Non-negative integer overrides only this type. 0 = never surface this type via RAG.': '留空 = 使用 RAG 设置里的全局默认。填非负整数则只覆写当前这一类。填 0 = 该类型永远不通过 RAG 召回出现。',
        '(use default)': '（使用默认）',
    ...
    addLocaleData('zh-tw', {
        ...
        'Empty = use the global default from RAG settings. Non-negative integer overrides only this type. 0 = never surface this type via RAG.': '留空 = 使用 RAG 設定裡的全局預設。填非負整數則只覆寫當前這一類。填 0 = 該類型永遠不通過 RAG 召回出現。',
        '(use default)': '（使用預設）',
