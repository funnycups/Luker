...
export function registerLocaleData() {
    addLocaleData('zh-cn', {
        ...
        'One-shot (full graph input)': '单次（全图输入）',
        'Crawl (LLM-explored local slice)': '爬取（LLM 按需探索局部切片）',
        'Crawl max rounds': '爬取最大轮数',
        'Crawl candidate limit': '爬取候选节点上限',
        'Crawl read budget': '爬取读取预算',
        'Crawl mode shows a compact graph index and reads relevant nodes on demand before extraction.': '爬取模式先展示紧凑图索引，LLM 按需读取相关节点后再进入正式抽取。',
        'About Extraction graph mode': '关于生成图模式',
        'Extraction graph mode help body': '选择生成图阶段的输入模式。单次 = 把当前可见图整体发给抽取 LLM；爬取 = 先由 LLM 用只读工具探索相关节点，只把爬到的局部切片发给抽取 LLM。',
        ...
        'Crawl read budget help body': '整个爬取阶段允许执行的最大读取操作数（inspect/neighbors/search 合计）。超出后提示 LLM 结束探索。',
        'Extraction Crawl Prompt': '生成图爬取提示词',
        'About Extraction Crawl Prompt': '关于生成图爬取提示词',
        'Extraction Crawl Prompt help body': '爬取阶段 LLM 的 system prompt。定义"你是只读探索器、从候选索引出发、按需读取节点、结束后调用完成工具"的角色和规则。',
    ...
    addLocaleData('zh-tw', {
        ...
        'One-shot (full graph input)': '單次（全圖輸入）',
        'Crawl (LLM-explored local slice)': '爬取（LLM 按需探索局部切片）',
        'Crawl max rounds': '爬取最大輪數',
        'Crawl candidate limit': '爬取候選節點上限',
        'Crawl read budget': '爬取讀取預算',
        'Crawl mode shows a compact graph index and reads relevant nodes on demand before extraction.': '爬取模式先展示緊湊圖索引，LLM 按需讀取相關節點後再進入正式抽取。',
        'About Extraction graph mode': '關於生成圖模式',
        'Extraction graph mode help body': '選擇生成圖階段的輸入模式。單次 = 把當前可見圖整體發給抽取 LLM；爬取 = 先由 LLM 用唯讀工具探索相關節點，只把爬到的局部切片發給抽取 LLM。',
        ...
        'Crawl read budget help body': '整個爬取階段允許執行的最大讀取操作數（inspect/neighbors/search 合計）。超出後提示 LLM 結束探索。',
        'Extraction Crawl Prompt': '生成圖爬取提示詞',
        'About Extraction Crawl Prompt': '關於生成圖爬取提示詞',
        'Extraction Crawl Prompt help body': '爬取階段 LLM 的 system prompt。定義「你是唯讀探索器、從候選索引出發、按需讀取節點、結束後呼叫完成工具」的角色和規則。',
        'RPM limit (0 = unlimited)': 'RPM 限制（0 = 不限制）',
