...
export function registerLocaleData() {
    addLocaleData('zh-cn', {
        ...
        'Persistent injection: keep only latest K per type (0 = no limit)': '常驻注入：每类型仅保留最新 K 个（0 = 不限制）',
        ...
        'About Persistent injection max per type': '关于常驻注入每类型上限',
        'Persistent injection max per type help body': '0 = 每类型的所有常驻节点都注入（当前行为）。K > 0 时，每个常驻类型按 seqTo 从新到旧只保留最近 K 个进入主上下文，更老的同类节点会从常驻注入中剔除，但仍在召回候选池里可被召回选中。event_table 也参与此限制；latest-only 类型（如角色卡、地点）例外——它们本身就只保留一条当前真相快照。适合事件密度大、想节省主上下文但仍保留召回能力的场景。',
    ...
    addLocaleData('zh-tw', {
        ...
        'Persistent injection: keep only latest K per type (0 = no limit)': '常駐注入：每類型僅保留最新 K 個（0 = 不限制）',
        ...
        'About Persistent injection max per type': '關於常駐注入每類型上限',
        'Persistent injection max per type help body': '0 = 每類型的所有常駐節點都注入（當前行為）。K > 0 時，每個常駐類型按 seqTo 從新到舊只保留最近 K 個進入主上下文，更老的同類節點會從常駐注入中剔除，但仍在召回候選池裡可被召回選中。event_table 也參與此限制；latest-only 類型（如角色卡、地點）例外——它們本身就只保留一條當前真相快照。適合事件密度大、想節省主上下文但仍保留召回能力的場景。',
