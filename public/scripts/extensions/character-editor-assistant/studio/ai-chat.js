...
This shape applies to anything where the **narrative position** is the primary thing the UI surfaces — chapter trackers in long-form story cards, mission/quest progression cards, arc indicators in episodic romance / slice-of-life cards, case-phase / case-name headers in investigation cards. If the card *also* has a graph-shaped layer (e.g. an investigation card with a phase header **and** suspect / clue / evidence-site nodes), use both: chat variables for the header, memory-graph schema for the entities. The two layers don't conflict, and the producer-side mistake is the same in either layer — wire the consumer, forget to instruct the AI to write.
