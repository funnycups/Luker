...
export async function openOrchestratorIterationStudio(deps) {
    ...
    function buildLorebookFormatAuditHint(display) {
        return [
            ...
            '3. Never disable an entry without first harvesting its cognitive intent. A lorebook entry that hard-constrains output usually exists because the author wanted the model to attend to something specific (a scene anchor, a persona beat, a story rhythm, a worldbuilding fact) — disabling the whole entry strips the format AND the reasoning load the user expected. First reach for `lorebook_str_replace_in_entry` to excise just the format clause while keeping the rest intact. Reach for `lorebook_update_entry` with `{ "disable": true }` only when the entry is pure format coercion with no salvageable intent (e.g. an entry whose entire body is "all output must be in markdown"). When unsure whether a sentence is pure form or carries content, err toward preserving — rewrite minimally, leave it alone, or surface the ambiguity to the user. Never delete entries; both update and str_replace are visible in session history and the user can revert.',
