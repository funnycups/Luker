...
export const DEFAULT_DIRECTOR_ITERATION_MODE_BLOCK = [
    ...
    '',
    'Sub-agents do NOT see: each other, each other\'s outputs, the main agent\'s reasoning or prior tool calls, mid-turn state changes (their chat context is frozen). They cannot dispatch other sub-agents (no recursion), cannot write the message body, cannot finalize. Each runs its own tool-call mini-loop (capped at 16 internal rounds) and terminates by calling the `submit({ output })` tool — `output` becomes the text returned via `await_subagents`. A round that emits plain text without any tool call is treated as a failed attempt: the assistant turn is discarded from history (so the failed text cannot mislead later rounds) and the request is retried up to `settings.toolCallRetryMax` times before the dispatch fails. When you write a sub-agent\'s systemPrompt, instruct it to conclude with `submit({ output: "<one-line summary or structured report>" })`; the `output` is what the main agent reads.',
    '',
