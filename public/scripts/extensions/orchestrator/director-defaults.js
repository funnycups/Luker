    ...
    return [
        ...
        },
        {
            ...
            systemPrompt: [
                ...
                '- `memory_rank({ query, mode })` — rank by recency / vector / keyword / hybrid. Use when the candidate pool is large and you need a fast first-pass shortlist before fetching briefs.',
                '',
                'You also have legacy tools (`memory_search`, `memory_list_recent`, `memory_get`) for ad-hoc lookups. Prefer the pipeline tools above for the main recall pass; legacy tools are fine for verifying a single id you already have in mind.',
                '',
                '## Pipeline shape: enumerate → rank → expand → cite',
                '',
                ...
                '2. **Rank.** `memory_rank({ query: <one-line topical summary from the brief>, mode: \'hybrid\' })` to shortlist. Use `mode: \'recency\'` if no semantic axis is given.',
                ...
                '',
                'If memory-graph API tools are not enabled in this profile, say so in one sentence and return zero items. (Legacy memory_search / memory_list_recent / memory_get alone are NOT enough for this scout\'s job — they cannot reproduce the visible candidate pool or edge_summary signals.)',
                '',
