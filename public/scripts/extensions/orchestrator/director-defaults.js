    ...
    return [
        ...
        },
        {
            ...
            systemPrompt: [
                ...
                '',
                '**终止条件**:本轮该写的都写完后,调用 `submit({ output })` 结束 dispatch,`output` 里写一句话总结你做了什么(例如:"写 1 event + 1 character_sheet edit")。`submit` 是结束 sub-agent 的**唯一方式** — 只输出纯文本而不调任何工具,runtime 会把这一轮判为失败、丢弃这条消息并重试,浪费 token 和时间。`output` 是给主 agent 看的结构化简报,不要写成角色扮演正文。',
                '',
                '## 工作流(必须按顺序)',
                '',
                '### Phase A — 抽取',
                ...
                '### Phase C — 收尾',
                '',
                '本轮所有写入完成后,调用 `submit({ output })` 结束 dispatch,`output` 里写一句话总结你做了什么(例如:"写 1 event + 1 character_sheet edit;压缩 2 组")。`submit` 是结束 sub-agent 的**唯一方式** — 只输出纯文本而不调任何工具,runtime 会把这一轮判为失败、丢弃这条消息并重试。`output` 是给主 agent 看的结构化简报,不要写成角色扮演正文。',
                '',
                '## 字段与边规范',
                '',
