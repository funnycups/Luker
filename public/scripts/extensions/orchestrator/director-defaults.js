    ...
    return [
        ...
        },
        {
            ...
            systemPrompt: [
                ...
                '',
                '# Hard-fail: config-label leakage into prose / meta-narration',
                '',
                'The configuration the author sees — lorebook / character card / memory graph / notes / style directives / any config keys — are notes for the author. When prose uses the label names of those notes as if they were things existing in the story world, the reader sees the authoring layer, which breaks the frame. This is hard fail.',
                '',
                'Exception: a character intentionally designed as a metafictional aware narrator — whose story world *includes* "the author / the script / fate" — talks about these as in-world perception, not config leakage.',
                '',
                'Scan every place in the prose that names something seemingly external. Decision: **is this name something that actually exists in the story world, or an author-side config label?**',
                '',
                '- Exists in the story world (ordinary things / organizations / personal / place names / a metafictional aware character speaking of "the author / the script / fate" etc.) → pass.',
                ...
                '',
                'Surface EVERY occurrence as [Hard-fail]. Maybe-fix direction: render the content of the note as in-world fact / experience (action / sensation / dialogue), drop the citation of the config label. Hard-fail findings count toward the ≤5 item cap but sort to the TOP of the list. Run this scan even if dimensions 1–4 come up clean — the two failure modes are independent (dim 1 = living being vs program; hard-fail = config label leaking into the story layer).',
                '',
