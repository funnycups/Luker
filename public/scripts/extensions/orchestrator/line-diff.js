/**
 * Side-by-side line-diff renderer for orchestrator UI.
 *
 * Owns the LCS-based line/word diff used in two places:
 *   - The AI iteration "before/after" diff for proposed profile edits
 *   - The runtime-trace attempt panel's `Rerun diff` block
 *
 * The renderer produces a `<details>` block with a paired old/new HTML
 * table whose rows are classified as add / delete / modify / equal.
 * `LINE_DIFF_LCS_MAX_CELLS` caps the per-pair LCS work so a pathological
 * pair degrades to a flat delete-then-insert pair instead of stalling
 * the UI; lines longer than `LINE_DIFF_LONG_*` thresholds collapse the
 * `<details>` element by default.
 *
 * `sanitizeDiffPlaceholderValue` normalizes placeholder strings ("Not
 * set" / "未设置" / "未設定") to empty so they don't show up as concrete
 * "before" or "after" content. `formatDiffValue` is its public alias —
 * named separately so call sites read as "format" at call sites that
 * don't otherwise know about diff placeholders.
 *
 * `escapeHtml` is duplicated here as a private helper to keep this
 * module portable. main.js owns the canonical copy; both will collapse
 * into a shared `html-utils.js` helper if/when more modules want it.
 */
...
export function renderIterationLineDiffHtml(beforeValue, afterValue, fileLabel = 'field') {
