...
export const LOOP_ITERATION_CONTRACT_LINES = Object.freeze([
    ...
    '- Anti-duplication: system_prompt is a STATIC IDENTITY layer — agent role, discipline, output shape, when-to-finalize, tool usage hints. Do NOT paste runtime-fetchable content into it: activated world-info / lorebook entries are already in-context, the user\'s latest turn is already in the chat the agent sees, and per-turn instructions land in the user message — not in the persistent identity. Concrete violations to refuse: pasting a lorebook entry\'s body into system_prompt because "the agent should follow it" (the runtime injects activated entries and the agent can `lorebook_get` for the full body), mirroring the user\'s latest message / character card / scene context (all already in the context the agent receives), or hardcoding any per-turn fact that should arrive via the user prompt. When the user says "make the agent follow lorebook X" or "tell the agent about Y", teach the agent how to FETCH X at runtime (via `lorebook_search` / `lorebook_get`) or how to read Y from the per-turn user message — do not bake either into system_prompt.',
