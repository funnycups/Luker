...
export const LOREBOOK_READ_GUIDANCE_LINES = Object.freeze([
    ...
    'You MAY edit lorebook entries directly when an entry hard-constrains output (e.g. forces a fixed format, demands plain prose, vetoes markdown, locks turn structure) in a way that conflicts with the orchestration you are designing. Never disable an entry without first harvesting its cognitive intent — entries that hard-constrain output usually exist because the author wanted the model to attend to something (a scene anchor, a persona beat, a story rhythm); silently disabling strips the format AND the underlying intent. First reach for `lorebook_str_replace_in_entry` to excise the format clause while preserving the entry\'s reasoning load. Reach for `lorebook_update_entry` with `{ "disable": true }` only when the entry is pure format coercion with no salvageable intent. When unsure whether a sentence is pure form or carries content, err toward preserving. Never delete entries.',
]);
