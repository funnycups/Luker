...
export const LOREBOOK_READ_GUIDANCE_LINES = Object.freeze([
    ...
    'You MAY edit lorebook entries directly when an entry hard-constrains output (e.g. forces a fixed format, demands plain prose, vetoes markdown, locks turn structure) in a way that conflicts with the orchestration you are designing. Use `lorebook_update_entry` to overwrite fields (preferred for `disable: true` to silence a problematic entry without losing its text) and `lorebook_str_replace_in_entry` for surgical wording tweaks. Prefer disabling over deleting; the entry stays in the book so a future user can re-enable it.',
]);
