...
export function registerLocaleData() {
    addLocaleData('zh-cn', {
        ...
        'terminator': '终止工具',
        '(forced on)': '（强制启用）',
        'Loop mode runs a single agent that calls tools in a loop and finalizes when ready. Character override for loop profiles is not yet supported.': 'Loop 模式让单个 Agent 反复调用工具，自行决定何时调用 finalize 完成。Loop 配置当前仅支持全局，不支持角色卡覆写。',
        'Loop profile character override is not yet supported. Save the iteration to the global profile instead.': 'Loop 配置当前不支持角色卡覆写。请将迭代结果保存到全局配置。',
        'Persist runtime trace to disk (experimental, default off)': '将运行态轨迹持久化到磁盘（实验性，默认关闭）',
        'Experimental: persist runtime trace events to disk after each run. Currently a stub on most setups; on-demand JSONL download from the runtime trace popup is always available.': '实验性：每次运行后将轨迹事件写入磁盘。当前在多数环境下是占位实现；运行态轨迹弹窗里的按需 JSONL 下载始终可用。',
    });
    addLocaleData('zh-tw', {
        'Orchestrator': '多智能體編排',
        'Enabled': '啟用',
        ...
        'terminator': '終止工具',
        '(forced on)': '（強制啟用）',
        'Loop mode runs a single agent that calls tools in a loop and finalizes when ready. Character override for loop profiles is not yet supported.': 'Loop 模式讓單個 Agent 反覆呼叫工具，自行決定何時呼叫 finalize 完成。Loop 設定目前僅支援全域，不支援角色卡覆寫。',
        'Loop profile character override is not yet supported. Save the iteration to the global profile instead.': 'Loop 設定目前不支援角色卡覆寫。請將迭代結果保存到全域設定。',
        'Persist runtime trace to disk (experimental, default off)': '將執行態軌跡持久化到磁碟（實驗性，預設關閉）',
        'Experimental: persist runtime trace events to disk after each run. Currently a stub on most setups; on-demand JSONL download from the runtime trace popup is always available.': '實驗性：每次執行後將軌跡事件寫入磁碟。目前在多數環境下是佔位實作；執行態軌跡彈窗中的按需 JSONL 下載始終可用。',
    });
}
